...
namespace Mapbox.Map
{
	...
	public sealed class TileResource : IResource
	{
		...
		{
			return new TileResource(string.Format("{0}/{1}", MapUtils.NormalizeStaticStyleURL(styleUrl ?? "mapbox://styles/mapbox/satellite-v9"), id.QuadKey()));
		}
		...
		{
			return new TileResource(string.Format("{0}/{1}", MapUtils.TilesetIdToUrl(tilesetId ?? "mapbox.satellite"), id.QuadKey()));
		}
		...
		{
			return new TileResource(string.Format("{0}/{1}", MapUtils.TilesetIdToUrl(tilesetId ?? "mapbox.terrain-rgb"), id.QuadKey()));
		}
